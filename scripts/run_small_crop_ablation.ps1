...
if ($Epochs -ne 50) {
    throw 'This small-object crop screening pipeline is pre-registered for exactly 50 epochs.'
}

$ProjectRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$Python = Join-Path $ProjectRoot '.venv\Scripts\python.exe'
$ReportRoot = Join-Path $ProjectRoot 'reports\small_object_crop'
...
function Update-Status {
    ...
    [ordered]@{
        ...
        stage = $Stage
        message = $Message
        current_variant = 'full_with_small_object_crop'
        epochs = $Epochs
        process_id = $PID
...
try {
    ...
    foreach ($Name in @($RunName, "small_crop_eval_${Epochs}e", "small_crop_size_eval_${Epochs}e")) {
    ...
    Update-Status -State 'running' -Stage 'initializing' -Message 'Small-object crop pipeline started'
    Invoke-Logged -Label 'train' -LogPath (Join-Path $LogRoot 'small_crop_train.log') -Arguments @(
        'train.py',
        '--stage', 'full',
        '--weights', 'yolo11s.pt',
        '--small-object-crop',
    ...
    Invoke-Logged -Label 'validation' -LogPath (Join-Path $LogRoot 'small_crop_val.log') -Arguments @(
        'evaluate.py',
        '--stage', 'full',
        '--weights', $BestWeights,
        '--split', 'val',
        ...
        '--plots',
        '--project', $RunRoot,
        '--name', "small_crop_eval_${Epochs}e",
        '--output-json', (Join-Path $MetricRoot 'small_crop_val.json')
    )

    Invoke-Logged -Label 'size-validation' -LogPath (Join-Path $LogRoot 'small_crop_size_val.log') -Arguments @(
