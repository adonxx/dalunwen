...
if ($Epochs -ne 50) {
    throw 'The P5-lite gated-fusion screening pipeline is pre-registered for exactly 50 epochs.'
}
...
$RunName = 'p5lite_gated_seed0_50e'
$EvalName = 'p5lite_gated_seed0_50e_eval'
$SizeEvalName = 'p5lite_gated_seed0_50e_size_eval'
$ControlMetrics = Join-Path $ProjectRoot 'reports\nwd_loss_ablation\metrics\inner_wiou_val.json'
...
try {
    foreach ($Path in @($Python, (Join-Path $ProjectRoot 'yolo11s.pt'), $ControlMetrics, $GroundTruth)) {
    ...
    Invoke-Logged -Label 'train' -LogPath (Join-Path $LogRoot 'p5lite_gated_train.log') -Arguments @(
    ...
    Invoke-Logged -Label 'validation' -LogPath (Join-Path $LogRoot 'p5lite_gated_val.log') -Arguments @(
        ...
        '--output-json', (Join-Path $MetricRoot 'p5lite_gated_val.json')
    )
    Invoke-Logged -Label 'size-validation' -LogPath (Join-Path $LogRoot 'p5lite_gated_size_val.log') -Arguments @(
        ...
        '--output-json', (Join-Path $MetricRoot 'p5lite_gated_size_val.json')
    )
    Invoke-Logged -Label 'report' -LogPath (Join-Path $LogRoot 'p5lite_gated_report.log') -Arguments @(
        'scripts\generate_p5lite_gated_report.py'
    )
    Update-Status -State 'completed' -Stage 'done' -Message 'P5-lite gated-fusion 50-epoch screen and evaluations completed.'
