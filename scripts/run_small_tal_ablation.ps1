...
if ($Epochs -ne 50) {
    throw 'This small-aware TAL screening pipeline is pre-registered for exactly 50 epochs.'
}

$ProjectRoot = (Resolve-Path (Join-Path $PSScriptRoot '..')).Path
$Python = Join-Path $ProjectRoot '.venv\Scripts\python.exe'
$ReportRoot = Join-Path $ProjectRoot 'reports\small_aware_tal'
...
function Update-Status {
    ...
    [ordered]@{
        ...
        stage = $Stage
        message = $Message
        current_variant = 'full_small_tal'
        epochs = $Epochs
        process_id = $PID
...
try {
    ...
    $ExpectedRuns = @(
        $RunName,
        "small_tal_eval_${Epochs}e",
        "small_tal_size_eval_${Epochs}e"
    )
    foreach ($Name in $ExpectedRuns) {
    ...
    Update-Status -State 'running' -Stage 'initializing' -Message 'Small-aware TAL pipeline started'
    Invoke-Logged -Label 'train' -LogPath (Join-Path $LogRoot 'small_tal_train.log') -Arguments @(
        'train.py',
        '--stage', 'full_small_tal',
        '--weights', 'yolo11s.pt',
    ...
    Invoke-Logged -Label 'validation' -LogPath (Join-Path $LogRoot 'small_tal_val.log') -Arguments @(
        'evaluate.py',
        '--stage', 'full_small_tal',
        '--weights', $BestWeights,
        '--split', 'val',
        ...
        '--plots',
        '--project', $RunRoot,
        '--name', "small_tal_eval_${Epochs}e",
        '--output-json', (Join-Path $MetricRoot 'small_tal_val.json')
    )

    Invoke-Logged -Label 'size-validation' -LogPath (Join-Path $LogRoot 'small_tal_size_val.log') -Arguments @(
